...
- X: 1/代码 0/数据
- X = 1：代码段
    - C: 是否是依从代码段（是依从代码段应用程序就可以直接跳过来执行）
    - R: 是否可读
- X = 0: 数据段
    - E: 0 向上扩展 / 1 向下扩展
    - W: 是否可写
...
- gdtr（GDT Register） / 全局描述符表的起始位置和长度
...
- 只需要一个代码段
- 需要一个或多个数据段 / 栈段 / 数据段
- 加载到段寄存器中（保存一个描述符，相当于上面gdt数组的索引，但还有别的功能） / 校验特权级
...
地址回绕（8086>1M会取模回到1M的内存）

当CPU需要访问的物理地址超过了1MB时，若没有开启A20线，就会发生回卷（wrap-around）现象，其本质是将高位地址部分忽略掉，把超出1MB的部分回卷到低位地址处，从而导致程序出现错误。开启A20线后，就能够正确地访问超出1MB的内存空间。

打开A20线的方式：0x92 端口的第2位置为1
